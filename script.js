...
const replacePalette = () => {
 try {
  ...
  var colorsToChange = Object.values(dictRgb).map((c) => c[oppositeTheme + "Rgb"]);
  ...
  $("*").each(function (index, element) {
   filteredKeys.forEach(function (key) {
    ...
    if (colorsToChange.includes($(element).css(key))) {
     $(element).css(key, newColors[colorsToChange.indexOf($(element).css(key))]);
    }
   });
  });
 } catch (ex) {
...
const refreshNewJob = () => {
 var containerImages = getElementsByIds("container-image", "container-image-2");
 containerImages.forEach((containerImage) => {
  containerImage.style.backgroundImage = "url(" + jobJson.link_image + ")";
 });
...
window.onload = async function () {
 ...
 submit.addEventListener("click", async function (event) {
  ...
  try {
   ...
   const newResolution = calculateResolution(window.innerWidth, window.innerHeight);
   ...
   var response = await fetch("https://stridai-backend-nest-71a94d04a6b3.herokuapp.com/jobs/create", requestOptions).then((response) =>
    response.json()
   );
   ...
   var interval = setInterval(async function () {
    const checkNewJob = await fetch("https://stridai-backend-nest-71a94d04a6b3.herokuapp.com/jobs/" + response.id, {
     method: "GET"
    });
    ...
    if (checkNewJob.status === 200 && checkNewJobJson.palette) {
     ...
     window.history.pushState({}, "", "/?session=" + response.id);
     refreshNewJob();
 ...
 if (window.location.search) {
  ...
  const job = await fetch("https://stridai-backend-nest-71a94d04a6b3.herokuapp.com/jobs/" + session, requestOptions);
  if (job.status === 200) {
   try {
